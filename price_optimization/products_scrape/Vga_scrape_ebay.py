...
def insertEbayDetails(page_html):
    ...
    for container in containers:
        if count != 0:
            ...
            user_rating = main_user_rating.user_rating()

            mydict = {"name": name, "size": capacity, "price": price, "type": memory_type, "warranty": warranty, "slot": slot,
                      "image": image, "owner": "ebay", "chipset": chipset, "capacity": capacity, "user_ratings": user_rating, "link": link_more}

            # Call Sorting script to get rating
            ratings = Points_script.sorting_algorithm("vga", mydict)
