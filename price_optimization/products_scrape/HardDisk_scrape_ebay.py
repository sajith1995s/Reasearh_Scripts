    ...
    for container in containers:
        if count != 0:
            ...
            for array_item in array:
                if "Storage Capacity:" in array_item:
                    capacity = array[count1 + 1]
                elif "Model:" in array_item:
                    model = array[count1 + 1]
                count1 = count1 + 1
            ...
            mydict = {"name": name, "size": capacity, "price": price, "warranty": warranty, "image": image,
                      "owner": "ebay", "user_rating": user_rating, "link": link_more}

            # Call points script to get rating
            ratings = points_script.sorting_algorithm("hard_disk", mydict)
            mydict["ratings"] = ratings

            # Check whether the product is already in the database
            mydoc = mycol.find_one({"$and": [{"name": name}, {"size": capacity}]})
